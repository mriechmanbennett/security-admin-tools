# Licensed code

Credit and license information for third-party code used in this project

## Index

- [HtmlAgilityPack](#htmlagilitypack)
- [MSCatalogLTS](#mscataloglts)
- [WindowsOSBuild](#windowsosbuild)

## [HtmlAgilityPack](https://github.com/mriechmanbennett/WindowsOSBuild/tree/sat2-submodule/lib/HtmlAgilityPack.LICENCE)

Dependency inherited from [MSCatalogLTS](#mscataloglts) and [WindowsOSBuild](#windowsosbuild)

Maintained by [ZZZ Projects](https://zzzprojects.com/)

Link to project [https://github.com/zzzprojects/html-agility-pack/](https://github.com/zzzprojects/html-agility-pack/)

MIT License

## [MSCatalogLTS](https://github.com/mriechmanbennett/MSCatalogLTS/tree/sat2-submodule/LICENCE)

Powershell module for searching and downloading updates from [https://www.catalog.update.microsoft.com](https://www.catalog.update.microsoft.com).

Forked from [https://github.com/Marco-online/MSCatalogLTS](https://github.com/Marco-online/MSCatalogLTS)

MIT License

Copyright (c) 2024 Marco-online

## [WindowsOSBuild](https://github.com/mriechmanbennett/WindowsOSBuild/tree/sat2-submodule/LICENCE)

Powershell module for querying patch release information.

Forked from [https://github.com/AshleyHow/WindowsOSBuild](https://github.com/AshleyHow/WindowsOSBuild)

MIT License

Copyright (c) 2025 Ashley How

Copyright (c) 2018 Fredrik Wall
